...
namespace StoreApi.Service.Storage;
...
public sealed class FileStorageService : IFileStorageService
{
    ...
    public async Task<string> UploadFileAsync(IFormFile file)
    {
        ...
        using (var memoryStream = new MemoryStream())
        {
            ...
            string requestUri = $"{_serviceURL}/{_bucketName}/{fileName}";
            string host = new Uri(_serviceURL).Host;
            ...
            byte[] signingKey = GetSignatureKey(_secretKey, dateShort, _region, "s3");
            ...
            using (var request = new HttpRequestMessage(HttpMethod.Put, requestUri))
            {
                ...
                request.Headers.Authorization = new AuthenticationHeaderValue("AWS4-HMAC-SHA256",
                    $"Credential={_accessKey}/{dateShort}/{_region}/s3/aws4_request, SignedHeaders=host;x-amz-content-sha256;x-amz-date, Signature={signature}");
    ...
    public async Task<bool> RemoveFileAsync(string fileName)
    {
        ...
        using (var request = new HttpRequestMessage(HttpMethod.Delete, requestUri))
        {
            ...
            request.Headers.Add("x-amz-date", date);
            request.Headers.Authorization = new AuthenticationHeaderValue("AWS4-HMAC-SHA256",
                $"Credential={_accessKey}/{dateShort}/{_region}/s3/aws4_request, SignedHeaders=host;x-amz-date, Signature={signature}");
